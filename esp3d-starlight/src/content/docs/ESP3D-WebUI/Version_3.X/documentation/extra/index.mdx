...
![image](extra_setting.png)

* This panel allows users to view, create, and manage their extra content.

![image](extra_list.png)

### Scan for extensions, themes and language packs

Starting with WebUI 3.1, the Extra content panel includes a **Scan** feature that automatically discovers compatible extensions, themes and language packs on the filesystem:

- **Extensions** — scans `/extensions` (or root `/`) for `esp3dext-*` files and directories, reads their manifest, and filters by WebUI version and target system.
- **Themes** — scans `/themes` (or root `/`) for `theme-*` files, reads their manifest if present, and filters by version and target.
- **Language packs** — scans `/languages` (or root `/`) for `lang-*.json` files and reads their `_manifest` if present.

The scan shows each item's compatibility status. You can select multiple items and add them in one click with **Add selected**.
...
![image](extra_output.png)
4. Define the URL Address where the extra content is located.    
   Note: the URL syntax depends on the type of content you want to display.
...
5. Select the types of Extra Content
   ...
   * Extension:
    This type of extra content is also HTML-based, but it utilizes the [extension API](/esp3d-webui/version_3x/documentation/api/extensions/).
    The [extension API](/esp3d-webui/version_3x/documentation/api/extensions/) allows the extra content to inject CSS styles and interact with the main web UI through a  [extension API](/esp3d-webui/version_3x/documentation/api/extensions/).
   * Image:
