...
description: ESP3D 2.1.X command reference
---

### Conventions

Please note all commands are in format `[ESPxx]`. These first brackets `[]` are not optional.  
Most of the time giving no argument will return current configuration. If authentication is on, some commands will need admin password. They are recognized by the optional argument `pwd=<admin password>` in command line.
...
* Set/Get notification settings (type can be NONE, PUSHOVER, EMAIL, WHATSAPP)   
